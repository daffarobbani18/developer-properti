...
const regex1 = /const\s+loginRes\s*=\s*await\s+fetch\([\s\S]*?body:\s*JSON\.stringify\(\{\s*email:\s*["']sales@erp\.com["'][\s\S]*?\}\)[\s\S]*?\}\);[\s\n]*const\s+loginData\s*=\s*await\s+loginRes\.json\(\);[\s\n]*const\s+token\s*=\s*loginData\.token;/g;
const regex2 = /const\s+loginRes\s*=\s*await\s+fetch\([\s\S]*?body:\s*JSON\.stringify\(\{\s*email:\s*["']sales@erp\.com["'][\s\S]*?\}\)[\s\S]*?\}\);[\s\n]*const\s+loginData\s*=\s*await\s+loginRes\.json\(\);[\s\n]*if\s*\(!loginData\.token\)\s*return;/g;
const regex3 = /const\s+loginRes\s*=\s*await\s+fetch\([\s\S]*?["']sales@erp\.com["'][\s\S]*?\}\);[\s\n]*const\s+loginData\s*=\s*await\s+loginRes\.json\(\);[\s\n]*if\s*\(!loginData\.token\)\s*return;/g;
const regex4 = /const\s+loginRes\s*=\s*await\s+fetch\([\s\S]*?["']sales@erp\.com["'][\s\S]*?\}\);[\s\n]*const\s+loginData\s*=\s*await\s+loginRes\.json\(\);[\s\n]*const\s+token\s*=\s*loginData\.token;/g;
const regex5 = /const\s+loginRes\s*=\s*await\s+fetch\([\s\S]*?["']sales@erp\.com["'][\s\S]*?\}\);[\s\n]*const\s+\{\s*token\s*\}\s*=\s*await\s+loginRes\.json\(\);/g;
