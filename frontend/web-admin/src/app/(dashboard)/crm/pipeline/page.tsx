...
export default function PipelinePage() {
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-zinc-900">CRM — Pipeline</h1>
        ...
        {pipelineColumns.map((status) => {
          ...
          return (
            ...
            >
              {/* Column Header */}
              <div
                className={`rounded-t-xl bg-white/80 backdrop-blur-md border border-zinc-200/50 border-t-[3px] ${columnAccent[status]} px-4 py-3 shadow-sm`}
              >
              ...
              {/* Column Body */}
              <div className="mt-0 rounded-b-xl bg-zinc-50/50 border border-t-0 border-zinc-200/30 p-2 min-h-[400px] space-y-2">
                {columnLeads.length === 0 ? (
                  ...
                  columnLeads.map((lead) => (
                    <div
                      ...
                      className={`cursor-grab active:cursor-grabbing rounded-lg bg-white/90 backdrop-blur-sm border border-zinc-200/40 p-3 shadow-sm transition-all duration-200 hover:shadow-md hover:border-blue-300/50 hover:-translate-y-0.5 ${
        ...
        <DialogContent className="bg-white/95 backdrop-blur-md border-zinc-200/50 rounded-xl max-w-md">
          ...
          {selectedLead && (
            <div className="space-y-4 mt-2">
              ...
              <div className="grid grid-cols-2 gap-3 rounded-lg bg-zinc-50/80 border border-zinc-200/40 p-4">
                <div>
                ...
                <div>
                  <p className="text-xs text-zinc-400 mb-0.5">Minat</p>
                  <p className="text-sm font-medium text-zinc-700">
                    {selectedLead.minatUnit}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-zinc-400 mb-0.5">Sales</p>
                  <p className="text-sm font-medium text-zinc-700">
                    {selectedLead.salesPIC}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-zinc-400 mb-0.5">Tanggal</p>
                  <p className="text-sm font-medium text-zinc-700">
