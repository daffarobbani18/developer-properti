...
// 1. Update FieldHomeScreen.tsx
let homePath = 'src/screens/pengawas/FieldHomeScreen.tsx';
let homeContent = fs.readFileSync(homePath, 'utf8');

if (!homeContent.includes('useNotifications')) {
  homeContent = "import { useNotifications } from \"../../contexts/NotificationContext\";\n" + homeContent;
  
  // Replace the useState and data loading for notifications
  ...
  homeContent = homeContent.replace(
    /const \[projectData, notifications\] = await Promise\.all\(\[\s*getFieldProjects\(auth\),\s*getRoleNotifications\(auth\),\s*\]\);\s*setProjects\(projectData\);\s*setUnreadCount\(notifications\.filter\(\(item\) => !item\.isRead\)\.length\);/,
    'const projectData = await getFieldProjects(auth);\n    setProjects(projectData);'
  );
  
  // Also remove getRoleNotifications import if present
  homeContent = homeContent.replace(/,\s*getRoleNotifications\s*/, ' ');
  
  // Optionally, show critical count in the dot or nearby?
  // Let's modify the heroMetricValue to highlight critical
  homeContent = homeContent.replace(
    /\{ color: unreadCount > 0 \? c\.danger\.text : "#ffffff" \}/,
    '{ color: criticalCount > 0 ? c.danger.text : unreadCount > 0 ? c.warning.text : "#ffffff" }'
  );
  
  fs.writeFileSync(homePath, homeContent);
}

// 2. Rewrite FieldNotificationsScreen.tsx
const notifPath = 'src/screens/pengawas/FieldNotificationsScreen.tsx';

const newNotifContent = `// [UI-FIX-V3] Priority Action Feed Redesign - SIMDP Mobile
...
export function FieldNotificationsScreen(): React.JSX.Element {
  ...
  const { auth } = useAuth();
  const { notifications, unreadCount, criticalCount, isLoading, refresh, markAsResolvedLocally, markAsReadLocally } = useNotifications();
      ...
      >
        <LinearGradient colors={[c.primaryDark, "#020617"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.heroHeader}>
          <SafeAreaView edges={['top', 'left', 'right']} style={styles.heroSafeArea}>
            <View style={styles.heroTopRow}>
              ...
              <Text style={styles.heroHeaderTitle}>Awareness Center</Text>
              <View style={{ width: 44 }} />
            </View>
        ...
        <View style={styles.contentPad}>
          <View style={styles.filterRow}>
            ...
            >
              <Text style={[styles.filterBtnText, filter === "actionable" && styles.filterBtnTextActive]}>Needs Action</Text>
             ...
             {isLoading && notifications.length === 0 ? (
                ...
                <View style={styles.feedList}>
                  {displayList.map((item, index) => {
                    ...
                    const statusText = item.isResolved ? "Resolved" : item.isRead ? "Read" : "Needs Action";
...
const styles = StyleSheet.create({
  ...
  heroTopRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 20, marginBottom: 24 },
  ...
  heroStatsRow: { flexDirection: "row", marginHorizontal: 20, backgroundColor: "rgba(255,255,255,0.06)", borderRadius: 16, padding: 16, borderWidth: 1, borderColor: "rgba(255,255,255,0.1)" },
  heroStatItem: { flex: 1 },
...
fs.writeFileSync(notifPath, newNotifContent);
console.log('FieldHomeScreen and FieldNotificationsScreen updated.');
